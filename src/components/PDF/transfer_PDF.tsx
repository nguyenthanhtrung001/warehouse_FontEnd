...
export const handlePrintPDF = async (receipt: any, details: any[]) => {
  ...
  // Add header
  doc.setFontSize(20);
  doc.text('PHIẾU TRẢ NHẬP HÀNG', 105, 20, { align: 'center' });

  // Add sub-header with invoice info
  doc.setFontSize(12);
  doc.text(`Mã phiếu trả NCC: DH000${receipt.id}`, 20, 30);
  doc.text(`Ngày: ${format(new Date(receipt.date), 'dd/MM/yyyy - HH:mm:ss')}`, 20, 40);
  doc.text(`Nhà cung cấp: ${receipt.supplier}`, 20, 50);
  doc.text(`Nhân viên thực hiện: nv000${receipt.employee}`, 20, 60);
  ...
  doc.setFontSize(12);
  doc.text('ID lô', 20, 70);
  doc.text('Tên Sản Phẩm', 50, 70);
  ...
  details.forEach((detail, index) => {
    doc.text("MKH000"+String(detail.batchDetail_Id || 'N/A'), 20, yPosition); 
    doc.text(detail.nameProduct, 50, yPosition);
