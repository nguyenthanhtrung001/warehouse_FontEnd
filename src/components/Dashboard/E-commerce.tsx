...
import ChartProductSale12Month from "../Charts/ChartProductSaleFor12Month";
      ...
      try {
        ...
        results.forEach((result, index) => {
          if (result.status === 'fulfilled') {
            switch (index) {
              case 0:
                setRevenue(result.value.data || 0); // Giả sử API trả về { revenue: number }
                break;
              case 1:
                setReturnNotes(result.value.data || 0); // Giả sử API trả về { count: number }
                break;
              case 2:
                setProducts(result.value.data || 0); // Giả sử API trả về { count: number }
                break;
              case 3:
                setPurchasePrice(result.value.data.totalPurchasePrice || 0); // Giả sử API trả về { totalPurchasePrice: number }
                break;
              default:
                break;
            }
          } else {
            console.error('Lỗi khi gọi API:', result.reason);
          }
        });
      } catch (error) {
  ...
  return (
    <>
      ...
      <svg
        ...
        <CardDataStats title="Nhập hàng" total={purchasePrice !== null ? formatCurrencyVND(purchasePrice) : 'Đang tải...'} >
        <svg
            className="fill-primary dark:fill-white"
            width="20"
            height="22"
        ...
        <CardDataStats title="Sản phẩm" total={products !== null ? products.toLocaleString() : 'Đang tải...'} >
        <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="22"
        ...
        <CardDataStats title="Trả hàng" total={returnNotes !== null ? returnNotes.toLocaleString() : 'Đang tải...'} >
        <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="18"
      ...
      <div className="mt-4 grid grid-cols-12 gap-4 md:mt-6 md:gap-6 2xl:mt-7.5 2xl:gap-7.5">
        ...
        {/* <ChartProductSale12Month /> */}
        {/* <MapOne /> */}
        <div className="col-span-6 xl:col-span-8">
         
          <ApexChart month={month} year={year} />
          <TableProductTopSale1 />
        </div>
        <TopProductInventory />
      </div>
    </>
