  ...
  const handleInvoiceClick = async (invoice: any) => {
    try {
      if (selectedInvoice && selectedInvoice.id === invoice.id) {
        ...
        const response = await axiosInstance.get(API_ROUTES.RETURN_DETAILS_BY_INVOICE_ID(invoice.id));
  ...
  const handleCancel = async (invoiceId: number) => {
    try {
      ...
      if (result.isConfirmed) {
        await axiosInstance.delete(API_ROUTES.DELETE_RETURN_DETAILS_BY_INVOICE_ID(invoiceId));
        fetchInvoices();
        Swal.fire(
          'Hủy thành công!',
                  ...
                  <div className="col-span-4 px-2 font-bold flex justify-end">
                    {invoice.status === "Tồn tại trả" && (
                      <button
                        ...
                        className="bg-red text-white px-4 py-2 rounded mr-2 w-30"
                      >
                        Hủy trả
                      </button>
                    )}
